...
也可以在这里维护 AI 工作习惯、公司标准文件、视图命名规则前缀和临时关键词。多个前缀用英文逗号分隔，例如：
...
- 把模型健康报告、模型快照或 dry-run CSV 交给 AI 分析。
- 给 AI 明确安全边界：只分析，不直接生成修改模型脚本。
- 自动列出最近的报告文件，方便选择要发送给 AI 的内容。
- 自动带上 `系统设置` 中保存的 AI 工作习惯。
- 自动带上 `系统设置` 中选择或创建的公司标准 Markdown。
