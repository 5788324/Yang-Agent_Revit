...
const desktopFeatureSenderResult = patchDesktopFeatureSender(desktopFeatureSenderFile);
const desktopFeatureMainResult = patchDesktopFeatureMain(desktopFeatureMainFile);

if (!changed && desktopFeatureSenderResult === 'skipped-unavailable' && desktopFeatureMainResult === 'skipped-unavailable') {
  process.stdout.write('skipped-unavailable');
  process.exit(0);
}

if (!changed && (desktopFeatureSenderResult === 'skipped-unavailable' || desktopFeatureMainResult === 'skipped-unavailable')) {
  process.stdout.write('patched-with-skips');
  process.exit(0);
}
...
function Invoke-PatchAppAsar {
  ...
  $browserUseArgs = @(
    [string]$targets.BrowserUseFeatureHook
    [string]$targets.BrowserSidebarAvailability
    $(if ([string]::IsNullOrWhiteSpace($targets.DesktopFeatureSender)) { '__none__' } else { [string]$targets.DesktopFeatureSender })
    $(if ([string]::IsNullOrWhiteSpace($targets.DesktopFeatureMain)) { '__none__' } else { [string]$targets.DesktopFeatureMain })
  )
  $browserUse = Invoke-NodePatcher $nodePath $patchers.BrowserUse $browserUseArgs
